...
const Slider = (props: Partial<CarouselProps>) => {
  ...
  const { dots = true, dotPosition = 'bottom', delayTime = 3000 } = props;
  ...
  const changeSlide = (idx: number) => {
    ...
    listWrapRef.current.style.transition = `all linear 0.2s`
  }

  // 处理指示点，箭头
  const clickHandler = (options:Partial<ClickProps>)  => {
    switch(options.type) {
      case 'dots': 
  ...
  useEffect(() => {
    ...
    if (props.autoplay) {
      autoplayTimer.current = setInterval(autoPlay, delayTime)
    }
    return () => {
      clearInterval(autoplayTimer.current)
    }
  }, [props.autoplay, currentSlide, childrenCount, delayTime])
